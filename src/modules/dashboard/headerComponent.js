...
function PopOverComponent(props) {
  ...
  const handleUpload = (event) => {
    ...
    setFile({ raw: event.target.files[0], previewURL: URL.createObjectURL(event.target.files[0]) });
  };
  ...
  const savePicture = async (event) => {
    ...
    let logoData = {}
    if (file && file.raw)
    logoData = await uploadLogo(file?.raw)
    ...
    const [err, response] = await Utility.parseResponse(UserService.updateUser(req));
    ...
    <div className={classes.root}>
      <div className={classes.styleHeader}>
        <UserAvatar
          ...
          aria-describedby={id} type="avatar" onClick={handleClick}
        />
      </div>
      <PopOverRapper>
        <Popover id={id} onClose={handleClick} open={open} anchorEl={anchorEl}
          anchorOrigin={{
            vertical: 'bottom',
            horizontal: 'center',
          }}
        ...
        >
          <div className={classes.paper}>
            ...
            <ul className={classes.textStyle} style={{ cursor: 'pointer' }} onClick={props.changePassword}>Change Password</ul>
            <ul className={classes.textStyle} style={{ cursor: 'pointer' }} onClick={handlePopup}>Upload Photo</ul>
            <ul className={classes.textStyle} style={{ cursor: 'pointer' }} onClick={props.logout}>Logout</ul>
            {cancel && <UploadPhoto
