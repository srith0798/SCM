...
export default function AlertDetails() {
  ...
  const getAlert = async (request) => {
    ...
    const [error, response] = await utility.parseResponse(
      AlertService.getAlertDetails(requestData)
    );
    if (error) return;
    setLoader(false);
    setAlert(response);
    ...
    response &&
      ...
      response.destinations.map((dest) => {
  ...
  const updateAlert = async (request) => {
    ...
    const [error, response] = await utility.parseResponse(
      AlertService.updateAlert(requestData)
    );
    if (error) return;
  ...
  const getDestinations = async () => {
    ...
    response.map((dest) => {
      if (destinationIds.includes(dest.destinationId)) dest["checked"] = true;
        ...
        <Container>
          ...
          <CommonDiv>
            <RowData>
              ...
              <SubHeading>
                {loader === false
                  ? `${genericConstants.ALERT_TYPE_NAMES[alert.type]} in ${
                      alert?.target?.name
                    }`
                  : ""}
              </SubHeading>
            ...
            <RowContainer>
              ...
              {alert.status === true ? (
                <DisableButton
                  ...
                  onClick={() => disableAlert(alert.alertId, false)}
                >
                  Disable
                </DisableButton>
              ) : (
              ...
              {alert.status === false ? (
                <DisableButton
                  style={{ marginLeft: "4px" }}
                  onClick={() => disableAlert(alert.alertId, true)}
                >
                  Enable
                </DisableButton>
              ) : (
